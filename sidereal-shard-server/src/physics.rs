use bevy::prelude::*;
use bevy_rapier2d::prelude::{NoUserData, RapierPhysicsPlugin, Velocity};
...
impl Plugin for ShardPhysicsPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_plugins(RapierPhysicsPlugin::<NoUserData>::default())
            ...
            .add_systems(
                FixedUpdate,
                (process_physics, check_entity_boundaries)
                    .chain()
                    .run_if(in_state(ShardState::Ready)),
            )
...
fn configure_physics_timestep(physics_config: Res<PhysicsConfig>, mut time: ResMut<Time<Fixed>>) {
    let period = 1.0 / physics_config.physics_fps;
    info!(
        "Setting physics timestep to {} seconds ({} FPS)",
        period, physics_config.physics_fps
    );
...
fn process_physics(time: Res<Time>, entities: Query<Entity>) {
    // Log every few seconds to avoid flooding logs
    if time.elapsed_secs() % 10.0 < time.delta_secs() {
        info!(
            "Physics processing active - {} entities in simulation",
            entities.iter().count()
        );
    }
}
