...
fn setup(mut commands: Commands) {
    ...
    let config = ConnectionConfig::from_shared_channels(vec![
        ChannelConfig {
            channel_id: 0,
...
fn log_connection_status(
    ...
    if time.elapsed_secs_f64() % 5.0 < 0.01 {
        info!("RenetClient details: connected={}, connecting={}", 
              client.is_connected(), transport.is_connecting());
    }
}

fn send_test_messages(
