...
mod tests {
    ...
    fn app_with_timeline(current_tick: Tick) -> App {
        let mut app = App::new();
        app.insert_resource(TickDuration(Duration::from_millis(10)));

        let mut timeline = InterpolationTimeline::default();
        timeline.context.remote_send_interval = Duration::from_millis(100);
    ...
    #[test]
    fn idle_rebase_writes_current_component_to_newest_confirmed_value() {
        let mut app = app_with_timeline(Tick(30));
        let mut history = ConfirmedHistory::<TestComp>::default();
        history.push(Tick(0), TestComp(0.0));
        history.push(Tick(10), TestComp(10.0));
        let entity = app.world_mut().spawn((history, TestComp(4.0))).id();
    ...
    #[test]
    fn smart_drain_preserves_bracketing_pair_under_packet_loss_gap() {
        let mut app = app_with_timeline(Tick(15));
        let mut history = ConfirmedHistory::<TestComp>::default();
        history.push(Tick(0), TestComp(0.0));
        history.push(Tick(10), TestComp(10.0));
        history.push(Tick(20), TestComp(20.0));
        let entity = app.world_mut().spawn((history, TestComp(5.0))).id();
