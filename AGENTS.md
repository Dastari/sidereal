...
- New world-facing shaders and material paths must consume the Lighting V2 contract (`system.lighting.v2`) unless they are explicitly emissive-only, UI/tactical overlay, or documented as exempt. Lighting remains client-derived presentation state and must not write authoritative simulation state.
