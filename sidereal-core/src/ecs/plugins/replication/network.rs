...
use bevy_replicon_renet2::{
    ...
    renet2::{self, ConnectionConfig, RenetClient, RenetReceive, RenetSend, RenetServer},
...
pub fn setup_client_transport(
    ...
    let native_socket =
        NativeSocket::new(socket).map_err(|e| format!("Failed to create native socket: {}", e))?;
    ...
    let server_channels: Vec<ChannelConfig> = replicon_channels
        .server_channels()
        .iter()
        .enumerate()
        .map(|(index, channel)| {
            ...
            ChannelConfig {
                ...
                max_memory_usage_bytes: channel
                    .max_bytes
                    .unwrap_or(replicon_channels.default_max_bytes),
                send_type,
            }
        })
        .collect();

    let client_channels: Vec<ChannelConfig> = replicon_channels
        .client_channels()
        .iter()
        .enumerate()
        .map(|(index, channel)| {
...
pub fn setup_server_transport(
    ...
    // Bind server socket
    let socket =
        UdpSocket::bind(server_addr).map_err(|e| format!("Failed to bind server socket: {}", e))?;
