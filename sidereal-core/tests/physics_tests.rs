...
fn test_basic_movement() {
    ...
    assert!(
        ...
        "X position should move in positive direction, got {}",
        transform.translation.x
    );
    assert!(
        transform.translation.y > 0.2,
        "Y position should move in positive direction, got {}",
...
fn test_n_body_gravity() {
    ...
    assert!(
        velocity.linvel.length() > 0.01,
        "Gravity should cause some velocity, but got length = {}",
...
fn test_orbital_mechanics() {
    ...
    println!(
        "   - Distance from center: {:.2} (initial was 100.0)",
        distance
    );
    println!(
        "   - Orbit status: {}",
