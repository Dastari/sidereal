...
export const Route = createFileRoute('/api/account/characters/$playerEntityId')(
  {
    server: {
      handlers: {
        DELETE: async ({ request, params }) => {
          ...
          try {
            await deleteDashboardCharacter(
              prepared.session,
              params.playerEntityId,
            )
            return json({ accepted: true }, { headers: prepared.headers })
          } catch (error) {
            return json(
              {
                error:
