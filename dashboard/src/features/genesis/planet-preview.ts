...
export function buildGenesisPlanetPreviewUniforms(
  ...
  return {
    ...
    'params.world_light_primary_dir_intensity': vec4(
      sunDir[0] ?? 0,
      sunDir[1] ?? 0,
      sunDir[2] ?? 1,
      settings.sun_intensity,
    ),
    'params.world_light_primary_color_elevation': vec4(
      1,
      0.94,
      0.82,
      sunDir[2] ?? 1,
    ),
    'params.world_light_ambient': vec4(0.16, 0.18, 0.22, 1),
    'params.world_light_backlight': vec4(0.28, 0.38, 0.62, 0.28),
    'params.world_light_flash': vec4(1, 1, 1, 0),
    'params.world_light_local_dir_intensity': vec4(-0.35, 0.1, 0.93, 0),
    'params.world_light_local_color_radius': vec4(0.45, 0.62, 1, 0),
