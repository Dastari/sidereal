...
use game::{process_message_queue, shard_manager::{balance_sectors, handle_shard_connection}, ShardManager};
...
fn setup_game_world(mut commands: Commands) {
    ...
    let ships: Vec<_> = (0..MOCK_SHIPS)
        ...
        .map(|_| {
            ...
            let hull_width = rng.gen_range(30.0..100.0);
            let hull_height = rng.gen_range(20.0..60.0);
            
            // Generate a random ship name using faker
            let ship_name = if rng.gen_bool(0.5) {
            ...
            // Add a fancy prefix
            let prefix = match rng.gen_range(0..7) {
                0 => "USS",
                1 => "ISS", 
                2 => "SSV",
                3 => "RSV",
            ...
            for _ in 0..num_blocks {
                blocks.push(Block {
                    x: rng.gen_range(-hull_width/2.0..hull_width/2.0),
                    y: rng.gen_range(-hull_height/2.0..hull_height/2.0),
            ...
            (position, velocity, hull_width, hull_height, full_name, blocks)
        })
        .collect();
