...
pub fn initialize_universe_state(
...
pub fn update_global_universe_state(
    ...
    for (entity, position) in query.iter() {
        if let Some(cluster) = universe_state.active_clusters.get_mut(&position.cluster_coords) {
            ...
            universe_state.entity_locations.insert(entity, position.cluster_coords);
        }
    }
}
...
pub fn handle_cluster_assignment(
    ...
    let unassigned_clusters: Vec<ClusterInfo> = universe_state.active_clusters
        .iter()
    ...
    for (idx, cluster_info) in unassigned_clusters.iter().enumerate() {
        ...
        universe_state.shard_assignments
            .entry(shard.id)
        ...
        info!("Assigned cluster {:?} (ID: {}) to shard {}", 
    ...
    *last_check = current_time;
    ...
    for (cluster_coords, cluster) in &mut universe_state.active_clusters {
        ...
        if all_sectors_empty && all_sectors_timed_out && cluster.assigned_shard.is_some() {
            clusters_to_release.push((*cluster_coords, cluster.id, cluster.assigned_shard.unwrap()));
        }
    }
    ...
    for (cluster_coords, cluster_id, shard_id) in clusters_to_release {
        ...
        info!("Releasing empty cluster at {:?} (ID: {}) from shard {:?}",
              cluster_coords, cluster_id, shard_id);
        
        // Tell the shard server to release it
        cluster_management_events.send(ClusterManagementMessage::ReleaseCluster {
            cluster_id,
        });
