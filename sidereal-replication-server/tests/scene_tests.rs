...
mod scene_tests {
    ...
    fn test_physics_data_deserialization() {
        ...
        let physics_data =
            PhysicsData::from_json(&physics_json).expect("Failed to deserialize physics data");
    ...
    fn test_entity_creation_from_record() {
        ...
        entity_mut.insert(Transform::from_xyz(
            record.position_x,
            record.position_y,
            0.0,
        ));
            ...
            if let Some(physics_data) = PhysicsData::from_json(physics_value) {
                ...
                if let Some(rigid_body_type) = &physics_data.rigid_body_type {
                    match rigid_body_type.as_str() {
                        ...
                        "static" => {
                            entity_mut.insert(RigidBody::Fixed);
                        }
                        "kinematic" => {
                            entity_mut.insert(RigidBody::KinematicPositionBased);
                        }
                        _ => {
                            entity_mut.insert(RigidBody::Dynamic);
                        }
        ...
        assert!(
            ...
            "Entity should have a RigidBody component"
        );
        assert!(
            world.get::<Collider>(entity).is_some(),
            "Entity should have a Collider component"
        );
