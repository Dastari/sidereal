...
use sidereal_replication_server::database::{DatabaseError, DatabaseResult, EntityRecord};
...
mod mock_database_tests {
    ...
    async fn test_fetch_all_entities() {
        ...
        for entity in &entities {
            assert!(
                Uuid::parse_str(&entity.id).is_ok(),
    ...
    async fn test_fetch_entities_by_type() {
        ...
        assert!(
            players[0].id.contains('-'),
            "Player ID should be in UUID format"
        );

        let asteroids = mock_client
            .fetch_entities_by_type("asteroid")
            .await
            .unwrap();
        assert_eq!(asteroids.len(), 1, "Expected 1 asteroid entity");
        assert!(
            asteroids[0].id.contains('-'),
            "Asteroid ID should be in UUID format"
        );

        let stations = mock_client.fetch_entities_by_type("station").await.unwrap();
        assert_eq!(stations.len(), 1, "Expected 1 station entity");
        assert!(
            stations[0].id.contains('-'),
            "Station ID should be in UUID format"
        );
    ...
    async fn test_fetch_entity_by_id() {
        ...
        let result = mock_client
            .fetch_entity_by_id("00000000-0000-0000-0000-000000000000")
            .await;
    ...
    async fn test_create_entity() {
        ...
        let created = mock_client
            .fetch_entity_by_id(&new_entity_id)
            .await
            .unwrap();
    ...
    async fn test_update_entity() {
        ...
        let result = mock_client
            .update_entity("00000000-0000-0000-0000-000000000000", &player)
            .await;
        assert!(
            result.is_err(),
            "Expected error when updating unknown entity"
        );
    ...
    async fn test_delete_entity() {
        ...
        let result = mock_client
            .delete_entity("00000000-0000-0000-0000-000000000000")
            .await;
        assert!(
            result.is_err(),
            "Expected error when deleting unknown entity"
        );
