...
fn test_serialization() {
    ...
    assert_eq!(test_entity_id, same_id_entity, "update_entity should find and update the existing entity with matching ID");
    
    // Verify the serialized data is the same
    let same_id_entity_ref = app.world().get_entity(same_id_entity).expect("Updated entity should exist in world");
    let same_id_serialized = serialize_entity(same_id_entity_ref, &app.world());
    let same_id_json: Value = serde_json::from_str(&same_id_serialized).expect("Updated entity should deserialize to valid JSON");
    ...
    let modified_serialized = serde_json::to_string(&modified_json).expect("Modified JSON should serialize");
    ...
    assert_ne!(test_entity_id, new_entity, "With different ID, update_entity should create a new entity");
    
    // Check both entities exist in the world
    assert!(app.world().get_entity(test_entity_id).is_ok(), "Original entity should still exist");
    assert!(app.world().get_entity(new_entity).is_ok(), "New entity should exist");
    
    // Serialize the new entity
    let new_entity_ref = app.world().get_entity(new_entity).expect("New entity should exist in world");
    let new_entity_serialized = serialize_entity(new_entity_ref, &app.world());
    let new_entity_json: Value = serde_json::from_str(&new_entity_serialized).expect("New entity should deserialize to valid JSON");
    ...
    assert!(
        test_entity_json.get("sidereal::ecs::components::object::Object").is_some(),
        "Original entity should have Object component"
    );
    assert!(
        new_entity_json.get("sidereal::ecs::components::object::Object").is_some(),
