...
This keeps art direction and per-body tuning in Lua while Rust owns replication schema, render plumbing, and shader execution. Shader delivery is still data-authored via `sprite_shader_asset_id`; the planet and star material families currently share one stable bind/uniform contract while their visual logic is split into separate shader sources.
