...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let enable_stars_layer = false;
    let enable_flares_layer = false;
    let enable_background_gradient = params.space_bg_section_flags.w >= 0.5;
    ...
    let tinted = mix(composed, composed * params.space_bg_tint.rgb * 2.8, 0.20);
    let rgb = max(tinted, vec3<f32>(0.0));
